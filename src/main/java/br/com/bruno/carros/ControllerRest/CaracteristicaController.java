...
import br.com.bruno.carros.Repository.CaracteristicasRepository;
	...
	private CaracteristicasRepository caracteristicasRepository;
	 ...
	 @GetMapping(path = "consulta/{codigo}")
	    public ResponseEntity consultar(@PathVariable("codigo") Integer codigo) {
	        return caracteristicasRepository.findById(codigo)
	                .map(record -> ResponseEntity.ok().body(record))
	                .orElse(ResponseEntity.notFound().build());
	    }
	 
	 @PostMapping(path = "salvar")
	    public Caracteristicas salvar(@RequestBody Caracteristicas caracteristicas) {
